...
1. `Component`s themselves, which generally contain all of the context needed for a single-turn request. MObjects manage context using fields and methods, and Instructions have a grounding_context for RAG-style requests.
...
m = mellea.MelleaSession(
    backend=OllamaBackend(model_id=IBM_GRANITE_4_MICRO_3B)
...
or reset the whole conversation (empty the ChatContext):

<CodeGroup>

```python Python
m = new_session(ctx=ChatContext)
m.chat("hello")
m.chat("How are you?")
m.reset()
should_be_empty = m.last_output
```
