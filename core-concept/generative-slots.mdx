...
And they are easy and fun to write. Here is another example for grading an essay:

<CodeGroup>
```python Python
@generative
def grade_essay(essay: str) -> int:
    """Grades the provided essay for language and grammar.

    Args:
        essay: the text to be graded.

    Returns:
        int: a grade between 1 (worst) and 100 (best).
    """
...
<Note>
Keep in mind that LLMs are good hallucinators. The above example should not be used for actual grading.
</Note>

More examples of generative slots are provided in the [docs/examples](https://github.com/generative-computing/mellea/tree/main/docs/examples/generative_slots) directory on GitHub.
